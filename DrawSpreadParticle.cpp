...
return
	[-1, 1)
******************************/
double PARTICLE::get_SignedRandom()
{
...
void DRAW_SPREAD_PARTICLE::setup(const FFT& fft)
{
	...
	********************/
	...
	/********************
	********************/
	TextureImg.load("texture/particleTexture.png");
...
{
	for(int i = 0; i < NUM_CLOUDS; i++){
		VboSet__SpreadParticle.set_TexCoords(i * 4 + 0, 0, TextureImg.getHeight());
		VboSet__SpreadParticle.set_TexCoords(i * 4 + 1, 0, 0);
		VboSet__SpreadParticle.set_TexCoords(i * 4 + 2, TextureImg.getWidth(), 0);
		VboSet__SpreadParticle.set_TexCoords(i * 4 + 3, TextureImg.getWidth(), TextureImg.getHeight());
	}
}
